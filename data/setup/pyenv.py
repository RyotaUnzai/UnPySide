...
class Pyenv(AbstractSoftware):
    ...
    def install(self) -> None:
        ...
        print(f"Installing pyenv in '{install_dir}")
        ...
        print(f"Installed pyenv in '{install_dir}")

    def uninstall(self) -> None:
        install_dir = self.__install_dir

        if not install_dir.exists():
            print(f"Skipped uninstall pyenv because not installed '{install_dir}'")
            return

        print(f"Uninstalling pyenv from '{install_dir}'")

        shutil.rmtree(install_dir)

        print(f"Uninstalled pyenv from '{install_dir}'")
    ...
    def install_python(self, py_ver: str) -> None:
        ...
        print(f"Installing Python {py_ver} into pyenv")

        env = self.apply_environ()

        subprocess.run(f'"{self.__pyenv_bat}" install {py_ver}', env=env)

        print(f"Installed Python {py_ver} into pyenv")
    ...
    def uninstall_python(self, py_ver: str) -> None:
        ...
        print(f"Uninstalling Python {py_ver} from pyenv")

        env = self.apply_environ()

        subprocess.run(f'"{self.__pyenv_bat}" uninstall {py_ver}', env=env)

        print(f"Uninstalled Python {py_ver} from pyenv")
